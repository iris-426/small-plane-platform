...
#include "fcu_core/uwb.h" //uwb添加：自定义消息
...
static mavlink_command_long_t uwb_command_long;//uwb添加：定义标准mavlink消息的结构体
ros::Publisher uwb;//uwb添加：定义发布者对象
fcu_core::uwb uwb_pub;//uwb添加：定义发布消息
	...
	if(n){
		...
		for(int i=0; i<n; i++){
            ...
            case MAVLINK_MSG_ID_SET_POSITION_TARGET_LOCAL_NED://仅机载电脑接收该数据包
              ...
              // uwb添加
            case MAVLINK_MSG_ID_COMMAND_LONG:
              mavlink_msg_command_long_decode(&msg_received,&uwb_command_long);
              // printf("dis:%f\n",command.param3);
              uwb_pub.header.stamp = ros::Time::now(); //时间戳
              uwb_pub.tag1_id = uwb_command_long.param1;
              uwb_pub.tag2_id = uwb_command_long.param2;
              uwb_pub.distance = uwb_command_long.param3;
              uwb.publish(uwb_pub);
            break;   
