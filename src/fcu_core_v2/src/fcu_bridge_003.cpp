...
static string drone_ip = "192.168.0.203"; //ip
...
int main(int argc, char **argv) {
  ...
  ros::NodeHandle nh("~");
  nh.param("DRONE_IP", drone_ip, string("192.168.0.203"));
  ...
  mission=nh.subscribe<std_msgs::Float32MultiArray>("mission_003", 100, missionHandler, ros::TransportHints().tcpNoDelay());
  path_global = nh.advertise<nav_msgs::Path>("path_global_003", 100);
  motion=nh.subscribe<geometry_msgs::PoseStamped>("motion_003", 100, motionHandler, ros::TransportHints().tcpNoDelay());
  command = nh.advertise<std_msgs::Int16>("command",100);
  path_target_pub = nh.advertise<nav_msgs::Path>("path_target_003", 100);
