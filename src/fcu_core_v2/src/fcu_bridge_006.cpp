...
int main(int argc, char **argv) {
  ...
  odom_global = nh.advertise<nav_msgs::Odometry>("odom_global_006_pc",100);
  gnss_001=nh.subscribe<sensor_msgs::NavSatFix>("/fcu_bridge_001/gnss_global_001", 100, gnssHandler, ros::TransportHints().tcpNoDelay());
  odom=nh.subscribe<nav_msgs::Odometry>("odometry_006", 100, odomHandler, ros::TransportHints().tcpNoDelay());
  cmd=nh.subscribe<std_msgs::Int16>("command", 100, cmdHandler, ros::TransportHints().tcpNoDelay());
  mission=nh.subscribe<std_msgs::Float32MultiArray>("mission_006", 100, missionHandler, ros::TransportHints().tcpNoDelay());
  ...
  path_target_pub = nh.advertise<nav_msgs::Path>("path_target_006", 100);
